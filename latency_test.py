...
def simulate_user(num_runs):
    ...
    # 创建并启动线程
    image_thread = threading.Thread(target=simulate_image_to_text, args=(latencies, file_path, num_runs))
    sentiment_thread = threading.Thread(target=simulate_sentiment_analysis, args=(latencies, text, num_runs))
    translate_thread = threading.Thread(target=simulate_translation, args=(latencies, ZN_text, num_runs))
    text_to_speech_thread = threading.Thread(target=simulate_text_to_speech, args=(latencies, text, num_runs))

    threads.append(image_thread)
    threads.append(sentiment_thread)
    threads.append(translate_thread)
    threads.append(text_to_speech_thread)
